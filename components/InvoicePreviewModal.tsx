...
export const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({ invoice, onClose }) => {
    ...
    const isRental = !!invoice.propertyId && invoice.category === 'Aluguéis';
    const documentTitle = isRental ? 'Recibo de Aluguel' : 'Fatura';

    const handleDownloadXml = () => {
        if (!submission.xmlContent) return;
        const blob = new Blob([submission.xmlContent], { type: 'application/xml' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `NFe_${submission.customer.replace(/\s/g, '_')}_${invoice.id}.xml`;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-xl shadow-2xl shadow-black/20 dark:shadow-black/60 ring-1 ring-slate-900/5 dark:ring-white/10 w-full max-w-3xl flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center print-hide">
                    <h2 className="text-xl font-bold">Visualizar {documentTitle}</h2>
                ...
                <div className="overflow-y-auto p-6 print:overflow-visible" id="invoice-preview-content">
                        ...
                        <div className="text-left max-w-3xl mx-auto bg-white dark:bg-slate-900 p-8 rounded-lg border border-gray-200/50 dark:border-gray-800/50 print:border-none print:p-0">
                           <div className="flex justify-between items-start">
                                ...
                                <div className="text-right">
                                    <h3 className="text-2xl font-bold uppercase text-slate-500 dark:text-slate-400">{documentTitle}</h3>
                 ...
                 <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex flex-wrap justify-end gap-3 print-hide">
                    ...
                    {submission.xmlContent && (
                        <ActionButton icon={<XmlIcon />} isSecondary onClick={handleDownloadXml}>
                            Baixar XML
                        </ActionButton>
                    )}
                </div>
...
const XmlIcon = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg>;