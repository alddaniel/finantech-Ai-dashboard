...
interface ScheduleReceivableModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export const ScheduleReceivableModal: React.FC<ScheduleReceivableModalProps> = ({ isOpen, onClose, onConfirm, transaction, bankAccounts }) => {
        ...
        if (transaction) {
             if (transaction.status === 'Agendado' && transaction.scheduledPaymentDate) {
                ...
                if (dueDate < today) {
                    ...
                    setScheduledDate(transaction.dueDate);
                }
            }
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 space-y-4">
                        <p className="text-slate-600 dark:text-slate-300">
                            Você está agendando a receita: <strong className="text-slate-800 dark:text-slate-100">{transaction.description}</strong>.
                        </p>
                        <div>
                            <label htmlFor="scheduledDate" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Selecione a data para o agendamento do recebimento</label>
                            <input
                                type="date"
                                id="scheduledDate"
                                name="scheduledDate"
                         ...
                         <div>
                            <label htmlFor="bankAccount" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Conta para Crédito</label>
                            <select
                                id="bankAccount"
                                value={bankAccountId}
