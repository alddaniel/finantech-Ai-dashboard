...
export const Sidebar: React.FC<SidebarProps> = ({ 
    ...
    useEffect(() => {
        ...
        const financeViews: View[] = ['payable', 'receipts', 'bank_reconciliation', 'payment_schedule', 'receivable_schedule', 'cash_flow_records', 'payable_recurrences', 'recurrences'];
  ...
  return (
    ...
    <aside className={`fixed top-0 left-0 h-full flex z-30 transition-transform duration-300 md:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      {/* Primary Icon Bar */}
      <div className="w-20 bg-slate-100 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col">
        <div className="flex-shrink-0 p-4 h-20 flex items-center justify-center">
      ...
      {/* Secondary Panel */}
      <div className={`w-52 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 flex flex-col transition-all duration-300 ${(isCollapsed && activePrimaryCategory !== 'dashboard') ? 'ml-[-13rem]' : 'ml-0'}`}>
