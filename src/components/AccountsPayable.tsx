...
import type { Transaction, Contact, Property, DanfeData, BankAccount, ToastMessage } from '../types';
...
export const AccountsPayable: React.FC<AccountsPayableProps> = ({ selectedCompany, payables, setPayables, contacts, setContacts, properties, onOpenExpenseModal, onOpenConfirmPaymentModal, bankAccounts, startDate, endDate, groupingType = 'none', addToast }) => {
    ...
    const filteredTransactions = useMemo(() => {
        let transactions = payables.filter(t => t.company === selectedCompany);
        ...
        return transactions;
    }, [payables, selectedCompany, startDate, endDate]);
    ...
    const groupedTransactions = useMemo(() => {
     ...
     const handleSchedule = (transaction: Transaction) => {
        setTransactionToSchedule(transaction);
    };
    ...
    const handleViewDanfe = async (xmlContent: string) => {
        try {
            const parsedData = await parseXmlToDanfeData(xmlContent);
    ...
    const handleExportCsv = () => {
        const headers = ['Descrição', 'Fornecedor', 'Categoria', 'Centro de Custo', 'Vencimento', 'Valor Original', 'Juros', 'Multa', 'Valor Total', 'Status'];
        let rows: string[] = [];
        ...
        Object.entries(groupedTransactions).forEach(([groupName, transactions]) => {
            ...
            transactions.forEach(t => {
                const supplierName = contacts.find(c => c.id === t.contactId)?.name || 'N/A';
    ...
    return (
                            ...
                            <table className="min-w-full">
                                <thead className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data Pagamento</th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Meio de Pgto.</th>
                                            ...
                                            <Fragment key={groupName}>
                                                {groupingType !== 'none' && (
                                                    <tr className="bg-slate-100 dark:bg-slate-800">
                                                        <th colSpan={11} className="px-6 py-2 text-left text-sm font-semibold text-slate-700 dark:text-slate-200">
                                                ...
                                                {transactions.map((transaction) => {
                                                    ...
                                                    const supplierName = contacts.find(c => c.id === transaction.contactId)?.name || 'desconhecido';
                                                    return (
                                                        <tr key={transaction.id} className="even:bg-gray-50 dark:even:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{transaction.status === 'Pago' ? formatDate(transaction.paymentDate) : '-'}</td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{transaction.status === 'Pago' ? (transaction.paymentMethod || '-') : '-'}</td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                                <Badge color={getStatusColor(transaction.status)}>{transaction.status}</Badge>
                                                            </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                                <div className="flex items-center gap-2">
                                                                    {transaction.purchaseXmlContent && (
                                                                        <>
                                                                            <button
                                                                                onClick={() => handleDownloadXml(
                                                                                    transaction.purchaseXmlContent!,
                                                                                    `NF_compra_${supplierName.replace(/\s/g, '_')}_${transaction.id}.xml`
                                                                                )}
                                                                            ...
                                                                            <button
                                                                                onClick={() => handleViewDanfe(transaction.purchaseXmlContent!)}
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                                ...
                                                                {transaction.status === 'Pendente' && (
                                                                    <button 
                                                                        onClick={() => handleSchedule(transaction)}
                                                                ...
                                                                <button 
                                                                    onClick={() => onOpenExpenseModal(transaction)}
                                                 ...
                                                 {groupingType !== 'none' && (
                                                     <tr className="bg-slate-50 dark:bg-slate-800/50 font-semibold">
                                                        <td colSpan={5} className="px-6 py-2 text-right text-slate-600 dark:text-slate-300">Subtotal do Grupo:</td>
                                                        <td colSpan={6} className="px-6 py-2 text-left text-slate-800 dark:text-slate-100">{formatCurrency(subtotal)}</td>
                                                    </tr>
                                                 )}
                                            </Fragment>
                                        )
                             ...
                             {filteredTransactions.length === 0 && <p className="text-center py-8 text-gray-500 dark:text-gray-400">Nenhuma conta a pagar encontrada para esta empresa.</p>}
                        </div>
                    </CardContent>
                </Card>
            </div>
        ...
        >
            Tem certeza que deseja excluir a despesa <strong className="text-slate-800 dark:text-slate-100">"{itemToDelete?.description}"</strong>? Esta ação não pode ser desfeita.
        </ConfirmationModal>
        </>
    );
};
